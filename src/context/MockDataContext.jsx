...
const generateInitialEntries = () => {
  ...
  workers.forEach((worker, wIndex) => {
    // Generar historial de 21 días hacia atrás desde el final de esta semana
    for (let i = 0; i <= 21; i++) {
      const entryDate = new Date(endOfWeek);
      entryDate.setDate(endOfWeek.getDate() - i);

      // No registrar nada en el futuro para Juan (id 1), para que su vista de trabajador sea realista
      if (entryDate > today && worker.id === '1') continue;

      // Dejar libre el día de hoy para Juan y María así se puede probar el botón Registrar Entrada
      const isToday = entryDate.toDateString() === today.toDateString();
      if (isToday && (worker.id === '1' || worker.id === '4')) continue;
